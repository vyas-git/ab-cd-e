...
let posthog_master: Record<string, PostHog> & {
    init: (token: string, vz_name: string, config: Partial<PostHogConfig>, name: string) => void
}
...
const defaultConfig = (): PostHogConfig => ({
    ...
    enable_recording_console_log: true, // When undefined, it falls back to the server-side setting
...
const create_phlib = function (
    ...
    instance._init(token, vz_name, config, name, handleCallback('initComplete'))
...
export class PostHog {
     ...
     */
    init(token: string, vz_name: string, config?: Partial<PostHogConfig>, name?: string): PostHog | void {
        ...
        const instance: PostHog = create_phlib(token, vz_name, config, name, (instance: PostHog) => {
    ...
    __compress_and_send_json_request(
        ...
        const [data, _options] = compressData(decideCompression(), jsonData, options)
    ...
    _calculate_event_properties(event_name: string, event_properties: Properties): Properties {
        ...
        properties['vz_name'] = this.get_config('vz_name')
    ...
    posthog_master['init'] = function (
        ...
        if (name) {
            ...
            if (instances[PRIMARY_INSTANCE_NAME]) {
                ...
                instance = create_phlib(token, vz_name, config || {}, PRIMARY_INSTANCE_NAME, (instance: PostHog) => {
...
export function init_from_snippet(): void {
    ...
    _each(
        posthog_master['_i'],
        function (item: [token: string, vz_name: string, config: Partial<PostHogConfig>, name: string]) {
            if (item && _isArray(item)) {
                instances[item[3]] = create_phlib(...item)
            }
        }
    )
